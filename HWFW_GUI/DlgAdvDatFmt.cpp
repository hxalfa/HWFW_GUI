...
/************************************************************************/
/*  Huawei WHWH sub-software                                             */
/************************************************************************/
typedef struct _HuaWeiSubItemObject {
  //Whether the sub-software is initialized
  BOOL          bIsInit = FALSE;
  //Is the sub-software in UImage format?
  BOOL          bIsImage = FALSE;
#if SUBITEM_USE_OFFSET
  //The offset of the sub-item relative to the item data
  uint32_t      u32Offset = 0;
#endif
  //Total software size (HW_HDR + UIMG_HDR + UIMG_Data)
  size_t        u32TotalSize = 0;
  //Pointing to Huawei data (xxx || uimage hdr + uimage data)
...
static int ImportData_WHWH() {
  ...
  __try {
    ...
    if (GetOpenFilePath(hDlgFmt, wsTmp, MAX_PATH))
    {
      ...
      {
        SetTooltip(GetDlgItem(hDlgFmt, IDC_LBL_ADF_STATUS), L"Failed to open the file, error code:[%d]!", GetLastError());
        return -1;
      }
      ...
      if (dwDataSize > sizeof(UIMG_HDR)) {
        ...
        // Determine whether it is uImage
        if (lpImgHdr->ih_magic == IH_MAGIC_LE) {
          sizeData = EndianSwap32(lpImgHdr->ih_size);

          if (dwDataSize < sizeof(UIMG_HDR) + sizeData) {
            SetTooltip(GetDlgItem(hDlgFmt, IDC_LBL_ADF_STATUS), L"The uImage format data length is wrong!");
            return -2;
          }
      ...
      SetTooltip(GetDlgItem(hDlgFmt, IDC_LBL_ADF_STATUS), L"Importing WHWH data is complete.");
      UpdateDataView();
      return 1;
    }
...
static int ImportData_UIMG() {
  ...
  __try {
    ...
    if (GetOpenFilePath(hDlgFmt, wsTmp, MAX_PATH))
    {
      ...
      SetTooltip(GetDlgItem(hDlgFmt, IDC_LBL_ADF_STATUS), L"Importing UIMG data is complete.");
      UpdateDataView();
      return 1;
    }
...
#define CURRENT     (lpSubItem[index])
      ...
      if (ret < 0) {
        ...
        wsprintf(szText, L"Error constructing sub-software: [%s]:[%d] !", szTemp, ret);
...
#undef CURRENT
  ...
  if (result != 0)
    ...
    SetTooltip(GetDlgItem(hDlgFmt, IDC_LBL_ADF_STATUS), L"Saving WHWH data completed.");
...
static uint32_t EnumSubItem() {
  ...
  //Determine whether the header length meets
...
static int InitSubItem(PHWSUBITEM_OBJ lpCurrent, LPCVOID lpData) {
  ...
  if (lpHwHdr->u32RearSize >= sizeof(UIMG_HDR)) {
    ...
    if (lpImgHdr->ih_magic == IH_MAGIC_LE) {
      ...
      // Determine whether the length of UImage Data + UImage Header exceeds the limit
  ...
  // Mark the software as initialized
...
/************************************************************************/
/* Initialize each sub-item element                                     */
/************************************************************************/
...
static INT_PTR InitDlg(HWND hDlg, uint32_t nIndex) {
  ...
  if (ret != 0) {
    SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"Failed to obtain software information:[%d].", ret);
    return TRUE;
  }
  else
  {
    ...
    wsprintf(szText, L"Advanced software data format editing => [%s]", szPath);
  ...
  if (ret != 0) {
    SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"Failed to obtain the software data size:[%d].", ret);
    return TRUE;
  }
  ...
  if (u32DataSize <= sizeof(HW_HDR)) {
    SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"Software data length is too small!");
    return TRUE;
  }

  if (lpItemData == NULL) {
    SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"The software data pointer is wrong!");
    return TRUE;
  }
  ...
  if (ret != 0) {
    SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"Failed to parse sub-software:[%d]!", ret);
    return TRUE;
  }
  ...
  {
    Release();
    MessageBoxW(hDlg, L"Error parsing advanced data structures!", L"Warning", MB_ICONWARNING | MB_OK);
    EndDialog(hDlg, 0);
  ...
  if (u32DataSize < sizeof(HW_HDR) + hdrWHWH.u32RearSize)
  {
    ...
    MessageBoxW(hDlg, L"WHWH data length is greater than software data length!", L"Error", MB_ICONERROR | MB_OK);
    return (INT_PTR)TRUE;
  }
  */
  ...
  case WM_COMMAND:
  {
    ...
    if (lParam != 0)
    {
      ...
      case IDC_BTN_WHCHK:
      {
        ...
        if (lpCurrentItem) {
          ...
          if (u32CRC == lpCurrentItem->hdrHuaWei.u32RearCRC)
            ...
            SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"WHWH data CRC32 check is incorrect, the calculation result:0x%08X!", u32CRC);
        }
        else {
          SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"Invalid sub-item!");
        }
      ...
      case IDC_BTN_UBCH:
        if (lpCurrentItem && lpCurrentItem->bIsImage)
        {
          ...
          if (u32CRC == EndianSwap32(lpCurrentItem->hdrImage.ih_hcrc))
            SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"UIMG header CRC32 check is correct.");
          else
            SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"UIMG header CRC32 check is incorrect, the calculation result:%08X (LE)!", u32CRC);
        }
        break;
      ...
      case IDC_BTN_UBCD:
        if (lpCurrentItem && lpCurrentItem->bIsImage)
        {
          ...
          if (u32CRC == EndianSwap32(lpCurrentItem->hdrImage.ih_dcrc))
            SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"UIMG data CRC32 check is correct.");
          else
            SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"UIMG data CRC32 check is incorrect, the calculation result:%08X (LE)!", u32CRC);
        }
        break;
        ...
        if (lpCurrentItem && lpCurrentItem->bIsImage)
        {
          ...
          if (GetSaveFilePath(hDlg, wsTmp, MAX_PATH))
          {
            if (ExportToFile(wsTmp, lpCurrentItem->lpcImageData, EndianSwap32(lpCurrentItem->hdrImage.ih_size)))
              SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"Exporting UIMG data completed.");
            else
      ...
      case IDC_BTN_ADD_HW_ITEM:
      {
        ...
        if (GetOpenFilePath(hDlg, szFile, MAX_PATH))
        {
          ...
          if (!ImportFromFile(szFile, &lpData, &dwSize)) {
            SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"Failed to import file data!");
          ...
          if (lpNewSubItem == NULL) {
            SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"Failed to reallocate memory!");
            if (lpData != NULL) free(lpData);
            break;
          }
      ...
      case IDC_BTN_EXP_HW_ITEM:
      {
        ...
        if (GetSaveFilePath(hDlg, szFile, MAX_PATH)) {
          ...
          __try {
            ...
            if (ret < 0) {
              SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"Failed to construct the subobject: [%d]!", ret);
            }
            else {
              if (ExportToFile(szFile, lpData, ret)) {
                SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"Export sub-software successfully: [%s]", szFile);
              }
              else {
                SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"Failed to export sub-software !!!");
              }
            }
          }
          __finally {
      ...
      case IDC_BTN_DEL_HW_ITEM:
      {
        ...
        if (nSubItem <= 1) {
          SetTooltip(GetDlgItem(hDlg, IDC_LBL_ADF_STATUS), L"Unable to continue deleting sub-software. A software contains at least one sub-software.!");
          break;
        }
        ...
        for (uint32_t i = 0, index = 0; i < nSubItem; i++) {
          //Skip the subitems that need to be deleted
